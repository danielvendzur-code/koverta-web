...
def main():
    ...
    # Zo zdroja; koverta-2026.js je z neho len zmenšený (tools/zmensi-js.js).
    js = io.open(os.path.join(KOREN, 'assets', 'koverta-2026.zdroj.js'), encoding='utf-8').read()
    for povinne in ["kvMeraj('dopyt_odoslany', { dopyt_typ:", "kvMeraj('telefon_klik')"]:
        if povinne not in js:
            chyby.append('assets/koverta-2026.zdroj.js: chýba meranie ' + povinne + ' (názvy používa agentúra v GTM, nemeniť)')
    if chyby:
