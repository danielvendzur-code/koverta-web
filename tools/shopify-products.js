...
function records(){const p=pages(),defs=[['koverta','auto','K','Prístrešok Koverta','Prístrešok pre auto','koverta'],['zahrada','zahrada','Z','Záhradný prístrešok Koverta','Záhradný prístrešok','zahrada']],out=[];for(const [key,fam,mkey,base,type,cfg] of defs){const d=data(p[key]),m=d.models[mkey],colors=d.colors.filter(c=>c.std).map(c=>c.ral+' · '+c.name);for(let li=0;li<m.lengths.length;li++)for(let wi=0;wi<m.widths.length;wi++){const l=m.lengths[li],w=m.widths[wi],price=m.prices[li][wi],dim=mt(w)+' × '+mt(l)+' m',area=w*l/1e6,typical=fam==='zahrada'?'terasa a záhradný priestor':(w>=5000?'2 osobné autá':'1 osobné auto');const description=fam==='zahrada'?'<p>'+base+' '+dim+' vytvorí '+area.toLocaleString('sk-SK')+' m² krytého priestoru pre terasu alebo záhradu. Čistá oceľová konštrukcia Koverta sa vyrába na Slovensku a rozmer aj vyhotovenie vieme prispôsobiť konkrétnemu domu a pozemku.</p><p>Vyberiete si dostupnú farbu a doplnky, prípadne si riešenie najprv pozriete v 3D konfigurátore. Pred výrobou spolu preveríme umiestnenie, rozmery a pripravenosť podkladu.</p><p>Ak vám katalógový rozmer presne nevyhovuje, vieme pripraviť individuálne riešenie podľa zamerania.</p>':'<p>'+base+' '+dim+' je praktické riešenie pre '+typical+'. Získate '+area.toLocaleString('sk-SK')+' m² zastrešenej plochy v čistej oceľovej konštrukcii, ktorá prirodzene zapadne k modernému aj klasickému domu.</p><p>Koverta sa vyrába na Slovensku a každý prístrešok vieme prispôsobiť konkrétnemu domu, pozemku aj vašim požiadavkám. Vyberiete si farbu konštrukcie a dostupné doplnky, prípadne si môžete celý prístrešok najskôr pozrieť a upraviť v našom 3D konfigurátore.</p><p>Pred výrobou s vami preveríme umiestnenie, rozmery a pripravenosť podkladu. Prístrešok následne vyrobíme podľa potvrdeného riešenia, privezieme a odborne namontujeme.</p><p>Ak vám presne '+dim+' nevyhovuje, nemusíte robiť kompromis. Rozmer aj vyhotovenie vieme prispôsobiť konkrétnemu priestoru.</p>';out.push({fam,w,l,price,colors,handle:handle(fam,w,l),title:base+' '+dim,type,dim,area,typical,description,cfg:PUBLIC_ORIGIN+'/pages/nove-konfigurator?page='+cfg+'&w='+w+'&l='+l,media:media(fam)})}}const approved5x6=out.find(r=>r.fam==='auto'&&r.w===5000&&r.l===6000);if(approved5x6)approved5x6.description="<p>Prístrešok Koverta 5 × 6 m je priestranné riešenie pre pohodlné parkovanie dvoch osobných áut. Získate 30 m² zastrešenej plochy v čistej oceľovej konštrukcii, ktorá prirodzene zapadne k modernému aj klasickému domu.</p><p>Koverta sa vyrába na Slovensku a každý prístrešok vieme prispôsobiť konkrétnemu domu, pozemku aj vašim požiadavkám. Vyberiete si farbu konštrukcie a dostupné doplnky, prípadne si môžete celý prístrešok najskôr pozrieť a upraviť v našom 3D konfigurátore.</p><p>Rozmer 5 × 6 m patrí medzi najpraktickejšie dvojmiestne varianty — poskytuje dostatok krytej plochy pre dve bežné osobné vozidlá a zároveň zostáva rozmerovo vhodný pre väčšinu rodinných domov.</p><p>Pred výrobou s vami preveríme umiestnenie, rozmery a pripravenosť podkladu. Prístrešok následne vyrobíme podľa potvrdeného riešenia, privezieme a odborne namontujeme.</p><p>Ak vám presne 5 × 6 m nevyhovuje, nemusíte robiť kompromis. Rozmer aj vyhotovenie vieme prispôsobiť konkrétnemu priestoru.</p>";for(const r of out){r.related=out.filter(x=>x.fam===r.fam&&x.handle!==r.handle).sort((a,b)=>(Math.abs(a.w-r.w)+Math.abs(a.l-r.l))-(Math.abs(b.w-r.w)+Math.abs(b.l-r.l))).slice(0,3).map(x=>x.handle)}return out}
