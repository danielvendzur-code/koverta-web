...
(() => {
  ...
  function initKviz(root) {
    ...
    /* Prvá otázka je, čo človek rieši — nie kde to stojí. Podľa nej sa
       vetví zvyšok: kto rieši tienenie, nedostane otázku na počet áut a kto
       rieši kuchyňu, nedostane otázku na typ strechy. Otázka sa zobrazí len
       vtedy, keď dáva zmysel (`ked`). */
    const OTAZKY = [
      { id: 'ciel', text: 'Čo riešite?', volby: [
        { v: 'auto', t: 'Zastrešiť auto', p: 'prístrešok alebo carport' },
        { v: 'terasa', t: 'Zastrešiť terasu alebo sedenie', p: 'pevná strecha alebo lamely' },
        { v: 'tienenie', t: 'Tieniť pred slnkom a pohľadmi', p: 'ZIP rolety, panely, brisoleje' },
        { v: 'kuchyna', t: 'Vonkajšiu kuchyňu alebo gril', p: 'hliníková linka do exteriéru' },
        { v: 'vstup', t: 'Prestrešiť vstup alebo menšiu plochu', p: 'dvere, schody, technika' }
      ] },
      { id: 'auta', ked: (o) => o.ciel === 'auto', text: 'Koľko áut má byť pod strechou?', volby: [
        { v: '1', t: 'Jedno auto' },
        { v: '2', t: 'Dve autá' },
        { v: '3', t: 'Tri a viac' },
        { v: 'box', t: 'Auto a k tomu uzamykateľný sklad' }
      ] },
      { id: 'plocha', ked: (o) => o.ciel === 'terasa', text: 'Aká veľká je plocha?', volby: [
        { v: 'mala', t: 'Do 20 m²', p: 'stôl pre štyroch' },
    ...
    const ZNACKY = {
      koverta: '<img src="' + kvAdresa('znackaKoverta', './assets/koverta-mark.svg') + '" alt="Koverta" width="516" height="77" loading="lazy" decoding="async">',
      soltec: '<img src="' + kvAdresa('znackaSoltec', './assets/soltec-mark.png') + '" alt="Soltec" width="971" height="268" loading="lazy" decoding="async">'
    };

    /* Rozhodovanie. Vetví sa podľa toho, čo človek rieši; hladina ceny
    ...
    /* Zoznam otázok, ktoré pri doterajších odpovediach dávajú zmysel.
    ...
    const vykresli = () => {
      ...
      o.volby.forEach((v) => {
        ...
        b.addEventListener('click', () => {
          /* Zmena prvej odpovede zahodí tie, ktoré k nej patrili — inak by
             sa do výsledku dostal počet áut pri kuchyni. */
  ...
  /* --- Súhlas s meraním ---------------------------------------------------
     Vlastnú lištu tu nemáme. Cookie lištu rieši platené rozšírenie na strane
     obchodu; dve lišty vedľa seba by si odporovali a návštevník by nevedel,
     ktorá platí. Google Consent Mode v hlavičke ostáva a kým ho niekto
     nepovolí, sú všetky kategórie zamietnuté: Tag Manager sa načíta, ale
     žiadna značka nesmie zapisovať do prehliadača. */
  /* --- Video v úvode ------------------------------------------------------
...
(function () {
  ...
  function vloz(text) {
    ...
    if (!pole || !text) return;
    /* Človek už písal — nesiahame na to. Text, ktorý sme vložili sami
       (napríklad s inou farbou), sa ale smie vymeniť. */
    if (pole.value.trim() && pole.value !== pole.dataset.kAuto) return;
    pole.value = text;
    pole.dataset.kAuto = text;
    ...
    if (window.matchMedia('(hover: none)').matches) return;   // klávesnica by zakryla formulár
    window.setTimeout(() => {
