(() => {
      ...
      if (cfgRoot && BIO) {
        ...
        const paintDepth = (faces, camera) => {
          ...
          const { gl, program, host, surface, position, color, pattern, texUV, maxSide } = depthPainter;
          ...
          const fits = Math.min(1, maxSide / Math.max(1, canvas.clientWidth * ratio),
                                   maxSide / Math.max(1, canvas.clientHeight * ratio));
